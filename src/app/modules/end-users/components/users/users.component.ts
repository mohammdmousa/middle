  ...
  getUsers(page: number = 1): void {
    const subscribe = this.callApi
      ...
      .subscribe({
        ...
        error: (error) => {
          this.auth.handleError(error);
          this.showAlert('error', 'Error To Fetch Users ', 'error');
        },
      });
    this.Subscription.push(subscribe);
  }
    ...
    this.callApi
      ...
      .subscribe({
        next: () => {
          ...
          this.getUsers();
          this.showAlert('success', ' New User Add succesfly', 'success');
        },
        error: (error) => {
          this.showAlert('error', 'Error in Add New User ', 'error');
        },
      });
  }
    ...
    this.callApi
      ...
      .subscribe({
        next: () => {
          ...
          this.getUsers();
          this.showAlert('success', ' Editing User Is succesfly', 'success');
        },
        error: (error) => {
          console.error('Error details:', error);
          this.showAlert('error', 'Error In Editing User', 'error');
        },
      });
  }

  deleteUser(id: number): void {
    this.showConfirm('Are You Sure?', 'This User Will Be Deleted').then(
      (result) => {
          ...
          this.callApi
            ...
            .subscribe({
              next: () => {
                ...
                this.showAlert(
                  'success',
                  ' User Deleting succesfly ',
                  'success'
                );
              },
              error: (error) => {
                this.showAlert('error', ' Error In Delete User', 'error');
              },
            });
        }
      }
  ...
  filterUsers(): void {
    const subscribe = this.callApi
      ...
      .subscribe({
        ...
        error: (error) => {
          this.auth.handleError(error);
          this.showAlert('error', ' Error In Filtering User   ', 'error');
        },
      });
    this.Subscription.push(subscribe);
  }
